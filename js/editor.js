        ...
        if (this.useIDB && this.db) {
            return new Promise((resolve) => {
                ...
                notesArray.forEach(note => {
                    note._mode = mode; // ✨ STRICT SEPARATION TAG ✨
                    store.put(note);
                });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ✨ STRICT ISOLATION: PURE CLOUD LOAD ✨
    async function initGitHubMode(token) {
        loadFolders();
        // 1. Fetch only Github specific cached notes (No relation to Local mode notes)
        const githubCachedNotes = await StorageManager.getAllNotes('github') || [];

        if (githubCachedNotes.length > 0) {
            notes = githubCachedNotes;
            extractFoldersFromNotes();
            activeNoteId = localStorage.getItem('md_active_github') || notes[0]?.id;
            finishAppLoad();
        }

        // 2. Fetch fresh from Cloud via GitHub API
        ...
        if (success) {
            ...
            if (githubCachedNotes.length === 0) {
                ...
                // Merge ONLY Cloud-Cached and Fresh-Cloud notes
                let mergedMap = new Map();
                githubCachedNotes.forEach(n => mergedMap.set(n.path, n));
    ...
    // ✨ STRICT ISOLATION: PURE LOCAL LOAD ✨
    async function loadLocalMode() {
        loadFolders();
        // Fetch only strictly Local notes
    ...
    document.getElementById('btn-sync-current')?.addEventListener('click', async () => {
        ...
        btn.innerHTML = `<i data-lucide="loader" class="spin" style="width:16px;"></i> Syncing`;
        btn.disabled = true;
        if (window.lucide) lucide.createIcons();
        ...
        if (success) {
            try {
                ...
                if (res && res !== 'conflict') {
                    ...
                    await saveLocalState();
                    window.showToast(`<i data-lucide='check-circle'></i> ${currentNote.title} synced successfully!`);
